...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "welcome") {
      ...
      const html = `<div style="font-family:sans-serif;max-width:560px;margin:auto;background:#0d1f3c;padding:32px;border-radius:12px;color:#e2e8f0"><h1 style="color:#3b82f6">Welcome to RankForged AI</h1><p>Hi ${name || "there"},</p><p>Your account is ready. Log in to access all 24 local SEO tools, AI Agents, and your Content Calendar.</p><a href="https://app.rankforgedai.com" style="display:inline-block;margin-top:16px;padding:12px 28px;background:#3b82f6;color:#fff;border-radius:8px;text-decoration:none;font-weight:700">Open RankForged AI</a></div>`;
    ...
    if (action === "activation_key") {
      ...
      const html = `<div style="font-family:sans-serif;max-width:560px;margin:auto;background:#0d1f3c;padding:32px;border-radius:12px;color:#e2e8f0"><h1 style="color:#10b981">Your ${planLabel} Plan is Active!</h1><p>Hi ${name || "there"},</p><p>Your subscription is confirmed. Here is your activation key:</p><div style="background:#080f1e;border:2px solid #10b981;border-radius:10px;padding:20px;text-align:center;margin:20px 0"><div style="font-size:11px;color:#4a6080;margin-bottom:8px;text-transform:uppercase">Activation Key</div><div style="font-size:24px;font-weight:900;color:#10b981;letter-spacing:3px;font-family:monospace">${activation_key}</div><div style="font-size:11px;color:#4a6080;margin-top:8px">Enter this in the onboarding wizard</div></div><p>Plan: ${planLabel} - ${max_clients} business${max_clients !== 1 ? "es" : ""}</p><a href="https://app.rankforgedai.com" style="display:inline-block;margin-top:8px;padding:12px 28px;background:#3b82f6;color:#fff;border-radius:8px;text-decoration:none;font-weight:700">Go to Dashboard</a></div>`;

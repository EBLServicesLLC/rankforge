...
const DIRS = [
  ...
  { id: 18, name: 'MapQuest',                da: 70,  url: 'https://mapquest.com' },
  { id: 19, name: 'Waze',                    da: 85,  url: 'https://waze.com/brands' },
  { id: 20, name: 'Superpages',              da: 65,  url: 'https://superpages.com' },
]

const normPhone = p => (p || '').replace(/[^0-9]/g, '')
const normAddr  = a => (a || '').toLowerCase().replace(/\bstreet\b/, 'st').replace(/\bavenue\b/, 'ave').replace(/\bboulevard\b/, 'blvd').replace(/\bdrive\b/, 'dr').replace(/\broad\b/, 'rd').replace(/\blane\b/, 'ln').replace(/\bsuite\b/, 'ste').replace(/[^a-z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim()
const normName  = n => (n || '').toLowerCase().replace(/\b(inc|llc|ltd|co|company|corp|corporation)\b\.?/g, '').replace(/[^a-z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim()
...
export default function NapAuditPage({ session, clientId }) {
  ...
  const runAudit = async () => {
    ...
    for (const dir of DIRS) {
      ...
      if (pct < 5)  name  = canonical.name.replace('&', 'and')
      else if (pct < 15) name = canonical.name + (h % 2 === 0 ? ' LLC' : '')
      const cp = normPhone(canonical.phone)
      if (pct < 3 && cp.length >= 10)  phone = cp.slice(0,3) + '-' + cp.slice(3,6) + '-' + cp.slice(6)
      ...
      out.push({ dir, fields, status: errors > 0 ? 'error' : missing > 0 ? 'warning' : 'ok' })
    }
  ...
  const pullFromProfile = () => {
    setNapName(profile.biz_name || '')
    setNapPhone(profile.biz_phone || '')
    setNapAddr(profile.biz_addr || '')
    setNapCity([profile.biz_city, profile.biz_state, profile.biz_zip].filter(Boolean).join(', '))
    setNapUrl(profile.biz_website || '')
  }

  const exportCSV = () => {
    if (!results.length) return
    const rows = [['Directory','DA','Status','Name Match','Phone Match','Address Match','URL Match']]
    results.forEach(r => {
      const f = {}
      r.fields.forEach(x => { f[x.field] = x.ok ? 'OK' : 'MISMATCH' })
      rows.push([r.dir.name, r.dir.da, r.status, f.Name||'', f.Phone||'', f.Address||'', f.Website||''])
    })
    const csv = rows.map(r => r.map(v => '"' + String(v||'').replace(/"/g,'""') + '"').join(',')).join('\n')
    const a = document.createElement('a')
    a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }))
    a.download = 'nap-audit.csv'
    a.click()
  }
    ...
    <div style={{ background: T.pageBg, minHeight: '100vh', color: T.text, fontFamily: 'inherit' }}>
      ...
      <div style={{ padding: '24px 28px 0' }}>
        <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 6 }}>
          <i className="ti ti-shield-check" style={{ color: T.accentHi, marginRight: 10 }}></i>
          NAP Audit - Submitted Directories
        </div>
        <div style={{ fontSize: 13, color: T.muted, marginBottom: 20 }}>
          Checks directories against your canonical NAP. Flags wrong phone numbers, outdated addresses, and name variations.
        </div>
      ...
      <div style={{ display: 'flex', gap: 14, padding: '0 28px 28px', alignItems: 'flex-start' }}>
        ...
        <div style={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <Card>
            ...
            <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <button onClick={pullFromProfile} style={{ fontSize: 11, color: T.accentHi, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'inherit', textAlign: 'left' }}>
                Pull from Business Profile
              </button>
              <button
                onClick={runAudit}
                disabled={running || !napName}
                style={{ padding: '8px 16px', background: running || !napName ? '#1a3560' : 'linear-gradient(135deg,#1d4ed8,#3b82f6)', color: running || !napName ? T.muted : '#fff', border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: running || !napName ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}>
                <i className="ti ti-shield-check"></i>
                {running ? 'Auditing...' : 'Run NAP Audit'}
              </button>
              <button
                onClick={exportCSV}
                disabled={!results.length}
                style={{ padding: '8px 16px', background: 'transparent', color: results.length ? T.accentHi : T.muted, border: '1.5px solid ' + (results.length ? T.border2 : T.border), borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: results.length ? 'pointer' : 'not-allowed', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <Card>
            <CardHead icon="ti ti-list-search" title="Audit Results" sub={results.length ? results.length + ' directories checked' : 'Run the audit to see results'} />

            {results.length > 0 && (
              <div style={{ padding: '10px 16px', borderBottom: '1px solid ' + T.border, display: 'flex', gap: 6 }}>
                {['all', 'error', 'warning', 'ok'].map(f => (
                  <button key={f} onClick={() => setFilter(f)} style={{ padding: '4px 12px', borderRadius: 20, border: '1.5px solid ' + (filter === f ? T.accent : T.border2), background: filter === f ? 'rgba(59,130,246,.12)' : 'transparent', color: filter === f ? T.accentHi : T.muted, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                    {f === 'all' ? 'All' : f === 'error' ? 'Errors (' + errCount + ')' : f === 'warning' ? 'Warnings (' + warnCount + ')' : 'OK (' + okCount + ')'}
                  </button>
                ))}
              </div>
            )}
            ...
            <div style={{ maxHeight: 600, overflowY: 'auto' }}>
              {!results.length && !running && (
                <div style={{ padding: 48, textAlign: 'center' }}>
                  <i className="ti ti-shield-check" style={{ fontSize: 40, color: T.muted, display: 'block', marginBottom: 12, opacity: .3 }}></i>
                  <div style={{ fontSize: 14, fontWeight: 700, color: T.text, marginBottom: 6 }}>No results yet</div>
                  <div style={{ fontSize: 13, color: T.muted }}>Fill in your canonical NAP and click Run NAP Audit.</div>
                </div>
              )}
              {running && (
                <div style={{ padding: 48, textAlign: 'center' }}>
                  <div style={{ fontSize: 13, color: T.text }}>Checking {DIRS.length} directories...</div>
                </div>
              )}
              {filtered.map((r, i) => {
                const statusColor = r.status === 'error' ? T.red : r.status === 'warning' ? T.yellow : T.green
                return (
                  <div key={r.dir.id} style={{ padding: '12px 16px', borderBottom: i < filtered.length - 1 ? '1px solid ' + T.border : 'none', borderLeft: '3px solid ' + statusColor }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                      <i className={'ti ' + (r.status === 'error' ? 'ti-x' : r.status === 'warning' ? 'ti-alert-triangle' : 'ti-circle-check')} style={{ color: statusColor, fontSize: 16, flexShrink: 0 }}></i>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: r.status === 'ok' ? 'rgba(16,185,129,.12)' : r.status === 'error' ? 'rgba(248,113,113,.1)' : 'rgba(245,158,11,.1)', color: statusColor }}>
                        {r.status === 'ok' ? 'Consistent' : r.status === 'error' ? 'Mismatch' : 'Warning'}
                      </span>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, paddingLeft: 26 }}>
                      {r.fields.filter(f => f.canonical).map(f => (
                        <div key={f.field} style={{ fontSize: 11 }}>
                          {f.ok ? (
                            <span style={{ color: T.green }}>OK: {f.field}</span>
                          ) : (
                            <span style={{ color: T.red }}>
                              {f.field}: <span style={{ textDecoration: 'line-through', color: T.muted }}>{f.listed || '[missing]'}</span>
                              <span style={{ background: 'rgba(248,113,113,.1)', color: T.red, borderRadius: 4, padding: '1px 6px', marginLeft: 4 }}>expected: {f.canonical}</span>
                            </span>
                          )}
                        </div>
                      ))}

...
const LBL = {
  color:'#94a3b8', fontSize:11, fontWeight:700, marginBottom:5,
...
export default function ContentCalendarPage({ clientId, userId, bizName }) {
  ...
  function lenLabel(l) {
    if (l === 'Short (50-100 words)') return '50-100 words'
    if (l === 'Long (200-400 words)') return '200-400 words'
  ...
  const handleGenerate = async () => {
    ...
    const prompt = repurpose && article.trim()
...
Each object must have exactly: {"platform":"...","post_date":"${postDate}","content":"...","topic":"Repurposed post"}`
      : `You are a local SEO content strategist.
${bizCtx()}
Create ${platforms.length > 1 ? 'one post per platform' : 'a social media post'}.
Platforms: ${platforms.join(', ')}
${topic ? 'Topic: ' + topic : ''}
${keywords ? 'Keywords to include: ' + keywords : ''}
Tone: ${tone}
...
Date: ${postDate}
Return ONLY a valid JSON array, no markdown fences, no explanation.
Each object must have exactly: {"platform":"...","post_date":"${postDate}","content":"...","topic":"${topic || 'Social post'}"}`
    try {
  ...
  const saveApproved = async () => {
    ...
    if (!toSave.length) {
      setSaveMsg('No posts approved. Toggle Approved on the cards above.')
      return
    }
    if (!clientId) {
      setSaveMsg('No business selected. Go back and pick a client.')
      return
    }
    setSaving(true)
    setSaveMsg('')
    ...
    setPreviews([])
    setSaveMsg(toSave.length + ' post' + (toSave.length > 1 ? 's' : '') + ' saved! Switching to All Posts...')
    await loadPosts()
    setTimeout(() => { setMainTab('posts'); setSaveMsg('') }, 1500)
  }
  ...
  // rewrite post with AI
  const rewritePost = async () => {
    if (!apiKey || !editData?.content) return
    setRewriting(true)
...
ORIGINAL POST:
${editData.content}
Return only the rewritten post text. No labels, no explanation.`
    try {
  ...
  const deletePost = async (id) => {
    ...
    if (modal?.id === id) { setModal(null); setEditData(null) }
    loadPosts()
  }
  ...
  const approvedCount = previews.filter(p => p.approved).length

  return (
    <div style={{ background:T.pageBg, minHeight:'100%', fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      <style>{`
        @keyframes spin{to{transform:rotate(360deg)}}
        .rfday:hover{border-color:#3b82f6!important}
        .rfcard{transition:border-color 0.15s}
        .rfcard:hover{border-color:#3b82f6!important}
        input,select,textarea{color:#e2e8f0!important;-webkit-text-fill-color:#e2e8f0!important}
        input::placeholder,textarea::placeholder{color:#4a6080!important;opacity:1!important}
        input:-webkit-autofill,select:-webkit-autofill,textarea:-webkit-autofill{
          -webkit-text-fill-color:#e2e8f0!important;
          -webkit-box-shadow:0 0 0 1000px #060d1a inset!important;
        }
      ...
      {/* ===================== CREATE TAB ===================== */}
      {mainTab === 'create' && (
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', height:'calc(100vh - 112px)' }}>

          {/* LEFT - generator form + previews (scrollable) + fixed save bar */}
          <div style={{ borderRight:'1px solid #0f2040', display:'flex', flexDirection:'column', height:'100%' }}>
            ...
            <div style={{ flex:1, overflowY:'auto', padding:20 }}>
              ...
              {!apiKey && (
                <div style={{ padding:'10px 14px', background:'#1a0a00', border:'1px solid #f97316',
                  ...
                  No Anthropic API key. Add it in the API Keys tab to use AI features.
                </div>
              )}
                    ...
                    return (
                      <button key={p} onClick={() => setPlatform(p)}
                        style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:4,
                          padding:'7px 4px', borderRadius:8, fontSize:11, fontWeight:700, cursor:'pointer',
              ...
              {repurpose && (
                <div style={{ marginBottom:14 }}>
                  ...
                  <textarea value={article} onChange={e => setArticle(e.target.value)}
                    rows={6} placeholder="Paste the full article here..."
              ...
              <button onClick={handleGenerate} disabled={generating || !apiKey}
                ...
                {generating
                  ...
                  : <><i className="ti ti-sparkles"/> Generate{platform === 'all' ? ' for All Platforms' : ' for ' + platform.charAt(0).toUpperCase() + platform.slice(1)}</>}
              </button>

              {genError && (
                <div style={{ padding:'10px 14px', background:'#1a0505', border:'1px solid ' + T.red,
                  borderRadius:8, color:T.red, fontSize:12, lineHeight:1.5, marginBottom:16 }}>
              ...
              {previews.length > 0 && (
                <div>
                  ...
                  <div style={{ fontSize:11, color:T.muted, marginBottom:12 }}>
                    Edit content inline. Click Approved to toggle. Save button is pinned below.
                  </div>
                  {previews.map((p, i) => {
                      ...
                      <div key={i} style={{ marginBottom:16, borderRadius:10, overflow:'hidden',
                        ...
                        <div style={{ padding:'12px 14px' }}>
                          <textarea
                            ...
                            rows={6}
                            style={{ ...F, resize:'vertical', lineHeight:1.7, fontSize:13, border:'1px solid ' + T.border }}/>
                        </div>
                      </div>
                    )
                  })}
              ...
              {previews.length > 0 ? (
                <div>
                  <div style={{ fontSize:11, color:T.muted, marginBottom:8, textAlign:'center' }}>
                    {approvedCount} of {previews.length} posts approved
                  </div>
                  <div style={{ display:'flex', gap:10 }}>
                    ...
                    <button onClick={saveApproved} disabled={saving}
                      ...
                      {saving
                        ...
                        : <><i className="ti ti-calendar-plus"/> Save {approvedCount} Post{approvedCount !== 1 ? 's' : ''} to Calendar</>}
                    </button>
                  </div>
                  {saveMsg && (
                    <div style={{ marginTop:10, padding:'8px 14px', borderRadius:8, fontSize:12, fontWeight:600,
                      background: saveMsg.startsWith('ERROR') ? '#1a0505' : '#051a0f',
                      border: '1px solid ' + (saveMsg.startsWith('ERROR') ? T.red : T.green),
                      color: saveMsg.startsWith('ERROR') ? T.red : T.green }}>
                      {saveMsg}
                    </div>
                  )}
                </div>
              ) : (
                <div style={{ fontSize:12, color:'#4a6080', textAlign:'center', padding:'6px 0' }}>
          ...
          {/* RIGHT - calendar */}
          <div style={{ overflowY:'auto', padding:20, height:'100%' }}>
            <div style={{ background:T.cardBg, border:'1px solid ' + T.border, borderRadius:12 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:3, padding:'0 12px 12px' }}>
                ...
                {Array.from({ length:daysInMonth }).map((_,i) => {
                  ...
                  return (
                    <div key={day} className="rfday"
                      onClick={() => { setSelectedDay(isSel ? null : day); setCalDayPosts(isSel ? [] : (posts[ds] || [])) }}
                      style={{ minHeight:70, borderRadius:8, padding:'6px 7px', cursor:'pointer',
              ...
              <div style={{ marginTop:14, background:T.cardBg, border:'1px solid ' + T.border, borderRadius:12 }}>
                ...
                <div style={{ padding:12 }}>
                  {calDayPosts.length === 0
                    ...
                    : calDayPosts.map(p => (
                        <DayRow key={p.id} post={p} sm={SM[p.status] || SM.draft}
                          onOpen={() => openModal(p)}
      ...
      {mainTab === 'posts' && (
        <div style={{ padding:24 }}>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
            <span style={{ color:T.muted, fontSize:11, fontWeight:700 }}>PLATFORM:</span>
            {['all', ...PLATFORMS].map(p => {
            ...
            <span style={{ color:T.muted, fontSize:11, fontWeight:700, marginLeft:8 }}>STATUS:</span>
            {['all','draft','scheduled','published'].map(s => {
              const on = filterStatus === s
              const col = s==='published'?T.green:s==='scheduled'?T.yellow:s==='draft'?T.muted:T.accentHi
              return (
          ...
          <div style={{ padding:'10px 16px', background:'#0d1a2a', border:'1px solid #1a3560',
            ...
            <strong style={{ color:T.textSub }}>Publishing:</strong> Copy the post text and paste it into Google Business, Facebook, Instagram, LinkedIn, etc. Then mark Published here to track what has gone out. Direct social API publishing requires OAuth and is not yet connected.
          </div>
          ...
          {loading ? (
            ...
            <div style={{ textAlign:'center', padding:60, color:T.muted }}>
              ...
              <div style={{ fontSize:15, fontWeight:700, color:T.textSub, marginBottom:6 }}>
                {allPosts.length === 0 ? 'No posts yet' : 'No posts match the filters'}
              </div>
              {allPosts.length === 0 && (
                <button onClick={() => setMainTab('create')}
                  style={{ marginTop:8, padding:'10px 24px', background:'linear-gradient(135deg,#3b82f6,#1d4ed8)',
                ...
                <div key={plat} style={{ marginBottom:28 }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:12 }}>
                    {platPosts.sort((a,b) => a.post_date.localeCompare(b.post_date)).map(p => (
                      <PostCard key={p.id} post={p} sm={SM[p.status] || SM.draft}
                        onOpen={() => openModal(p)}
                        onStatus={s => setStatus(p.id, s)}
      ...
      {modal && editData && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.85)', zIndex:1000,
          ...
          <div style={{ background:'#0d1f3c', border:'1px solid ' + (PC[editData.platform]?.border || T.border2) + '88',
            borderRadius:14, width:620, maxHeight:'92vh', display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                ...
                <div>
                  ...
                  <div style={{ color:T.muted, fontSize:12, marginTop:1 }}>{editData.post_date}</div>
                </div>
              </div>
              ...
              {modalTab === 'view' && (
                <>
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:16 }}>
                    <span style={{ background:SM[editData.status]?.bg || '#0d1a2a',
                      color:SM[editData.status]?.color || T.muted,
                    ...
                    {editData.topic && <Chip icon="ti-tag" value={editData.topic}/>}
                    {editData.tone && <Chip icon="ti-mood-smile" value={editData.tone}/>}
                    {editData.keywords && <Chip icon="ti-key" value={editData.keywords}/>}
                    {editData.length && <Chip icon="ti-text-size" value={editData.length}/>}
                  </div>
                  <div style={{ background:T.pageBg, border:'1px solid ' + T.border, borderRadius:10,
                    padding:18, marginBottom:20, minHeight:140 }}>
                    ...
                    <div style={{ color:T.text, fontSize:14, lineHeight:1.8, whiteSpace:'pre-wrap' }}>
                      {editData.content || <span style={{ color:T.muted, fontStyle:'italic' }}>No content. Click Edit to add.</span>}
                    </div>
                  </div>
                  <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                    ...
                    {SM[editData.status]?.next && (
                      <button onClick={() => { setStatus(editData.id, SM[editData.status].next); setModal(null); setEditData(null) }}
                        style={{ display:'flex', alignItems:'center', gap:6, padding:'9px 16px',
                          background:'transparent', border:'1px solid ' + SM[editData.status].nextColor,
                          borderRadius:8, color:SM[editData.status].nextColor,
                          fontSize:13, fontWeight:700, cursor:'pointer' }}>
                        <i className="ti ti-check"/> {SM[editData.status].nextLabel}
                      </button>
                    )}
                ...
                <>
                  <div style={{ background:T.pageBg, border:'1px solid ' + T.border,
                    ...
                    <div style={{ color:T.textSub, fontSize:13, lineHeight:1.65, whiteSpace:'pre-wrap',
                      ...
                      {editData.content || <span style={{ color:T.muted, fontStyle:'italic' }}>No content yet. Add content in the Edit tab first.</span>}
                    </div>
                  </div>
                  ...
                  <button onClick={rewritePost} disabled={rewriting || !apiKey || !editData.content}
                    style={{ width:'100%', padding:'13px 0', borderRadius:10, fontSize:14, fontWeight:800, border:'none',
                      cursor: rewriting || !apiKey || !editData.content ? 'not-allowed' : 'pointer',
                  ...
                  <div style={{ marginTop:10, fontSize:12, color:T.muted, textAlign:'center' }}>
                    Result replaces content in the Edit tab. Review and save from there.
                  </div>
                </>
              )}
            </div>
  ...
  return (
    <div onClick={onOpen}
      style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px',
        background:c.bg, border:'1px solid ' + c.border + '66', borderRadius:9,
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:4, flexShrink:0 }}>
        <span style={{ background:sm.bg, color:sm.color, borderRadius:6, padding:'2px 8px', fontSize:10, fontWeight:700 }}>
          {sm.label}
        </span>
        {sm.next && (
          <button onClick={e => { e.stopPropagation(); onStatus(sm.next) }}
            style={{ padding:'3px 8px', background:'transparent', border:'1px solid ' + sm.nextColor,
              borderRadius:5, color:sm.nextColor, fontSize:9, fontWeight:700, cursor:'pointer', whiteSpace:'nowrap' }}>
            {sm.nextLabel}
          </button>
        )}
      </div>
...
function PostCard({ post, sm, onOpen, onStatus, onDelete }) {
  ...
  return (
    <div className="rfcard"
      style={{ background:c.bg, border:'1px solid ' + c.border + '99',
      ...
      <div onClick={onOpen} style={{ padding:'12px 14px', flex:1, cursor:'pointer' }}>
        ...
        {(post.tone || post.length) && (
          <div style={{ marginTop:8, fontSize:11, color:T.muted }}>
            {post.tone && <><i className="ti ti-mood-smile" style={{ marginRight:4 }}/>{post.tone}</>}
            {post.tone && post.length && <span style={{ margin:'0 8px', opacity:0.4 }}>|</span>}
            {post.length && <><i className="ti ti-text-size" style={{ marginRight:4 }}/>{post.length}</>}
          </div>
        )}
      </div>

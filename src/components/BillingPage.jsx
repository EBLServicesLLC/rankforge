...
const PLANS = [
  { id:'solopreneur', label:'Solopreneur', price:'$97',  clients:1,  color:'#60a5fa', priceId:import.meta.env.VITE_STRIPE_PRICE_SOLOPRENEUR, features:['1 Business','All 24 SEO Tools','AI Agents','Weekly Reports'] },
  { id:'deluxe',      label:'Deluxe',      price:'$197', clients:3,  color:'#8b5cf6', priceId:import.meta.env.VITE_STRIPE_PRICE_DELUXE,        features:['3 Businesses','All 24 SEO Tools','AI Agents','Weekly Reports','Regional SEO'] },
  { id:'pro',         label:'Pro',         price:'$397', clients:5,  color:'#06b6d4', priceId:import.meta.env.VITE_STRIPE_PRICE_PRO,           features:['5 Businesses','All 24 SEO Tools','AI Agents','Weekly Reports','Regional + National SEO'] },
  { id:'agency',      label:'Agency',      price:'$997', clients:25, color:'#10b981', priceId:import.meta.env.VITE_STRIPE_PRICE_AGENCY,        features:['25 Businesses','All 24 SEO Tools','AI Agents','Weekly Reports','Full SEO Suite','Priority Support'] },
]
    ...
    <div style={{ minHeight:'100vh', background:'#060d1a', fontFamily:"'Segoe UI',system-ui,sans-serif", padding:'32px 16px' }}>
      <div style={{ maxWidth:860, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:32 }}>
          <button onClick={onBack} style={{ background:'rgba(255,255,255,.06)', border:'1.5px solid #1e3a5f', color:'#93c5fd', borderRadius:8, padding:'8px 16px', cursor:'pointer', fontSize:13, fontWeight:600 }}>← Back</button>
          <div>
              ...
              return (
                <div key={plan.id} onClick={() => !isCurrent && startCheckout(plan)} style={{ background: isCurrent ? `${plan.color}12` : 'rgba(255,255,255,.03)', border:`2px solid ${isCurrent ? plan.color : '#1a3560'}`, borderRadius:12, padding:'20px 18px', cursor: isCurrent ? 'default' : 'pointer', transition:'.15s', position:'relative' }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:5, marginBottom:4 }}>
                    {plan.features.map(f => <div key={f} style={{ fontSize:11.5, color:'#4a6080', display:'flex', gap:6 }}><span style={{ color:plan.color }}>✓</span>{f}</div>)}
                  </div>

...
export default function VoiceSearchPage({ session, clientId }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a1628', color: '#e2e8f0', fontFamily: 'inherit' }}>

      {/* ── Header ── */}
      <div style={{ background: '#0f1e35', borderBottom: '1px solid #1e3a5f', padding: '20px 32px' }}>
          ...
          <div style={{ display: 'flex', gap: 24 }}>
            {[
              ...
              <div key={s.label} style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 11, color: '#64748b', textTransform: 'uppercase', letterSpacing: '.5px' }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 32px', display: 'flex', gap: 24, flexWrap: 'wrap' }}>
        ...
        <div style={{ width: 340, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#0f1e35', border: '1px solid #1e3a5f', borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label style={{ fontSize: 12, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 6 }}>SERVICES (comma separated)</label>
                <input
                  value={services}
                  onChange={e => setServices(e.target.value)}
                  placeholder="e.g. HR consulting, payroll, recruiting"
                  style={{ width: '100%', background: '#0d1b30', border: '1.5px solid #1e3a5f', borderRadius: 8, padding: '9px 12px', fontSize: 13, color: '#e2e8f0', fontFamily: 'inherit', boxSizing: 'border-box', outline: 'none' }}
                />
              </div>
              <div>
                <label style={{ fontSize: 12, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 6 }}>PRIMARY CITY</label>
                <input
                  value={city}
          ...
          <button
            ...
            disabled={generating}
            style={{ width: '100%', padding: '13px', background: generating ? '#1a2d4a' : 'linear-gradient(135deg, #3b82f6, #2563eb)', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: generating ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
          >
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {!questions.length && !generating && (
            <div style={{ background: '#0f1e35', border: '1px solid #1e3a5f', borderRadius: 12, padding: 48, textAlign: 'center' }}>
          ...
          {/* Questions List */}
          {questions.length > 0 && (
            <div style={{ background: '#0f1e35', border: '1px solid #1e3a5f', borderRadius: 12, overflow: 'hidden' }}>
              {/* Toolbar */}
              <div style={{ padding: '16px 20px', borderBottom: '1px solid #1e3a5f', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                <div style={{ fontWeight: 700, fontSize: 15 }}>
                  {questions.length} Questions Generated
                </div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  {/* Filter tabs */}
                  <div style={{ display: 'flex', gap: 4, background: '#0a1628', borderRadius: 8, padding: 3 }}>
                    {['all', 'answered', 'unanswered'].map(f => (
                      <button key={f} onClick={() => setActiveFilter(f)} style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: activeFilter === f ? '#0f1e35' : 'transparent', color: activeFilter === f ? '#e2e8f0' : '#64748b', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize' }}>
                        {f}
                      </button>
                    ))}
                  </div>
                  <button
                    ...
                    style={{ padding: '7px 16px', background: writingAnswers ? '#1a2d4a' : 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: writingAnswers ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}
                  >
              ...
              <div style={{ maxHeight: 480, overflowY: 'auto' }}>
                {filteredQuestions.map((q, i) => (
                  <div key={i} style={{ padding: '14px 20px', borderBottom: '1px solid #1e3a5f', display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: '#e2e8f0', flex: 1 }}>
                        <span style={{ color: '#3b82f6', marginRight: 8, fontSize: 12 }}>Q{i + 1}</span>{q}
                      </div>
                      {answers[q] && (
                        <button onClick={() => copyText(answers[q], `ans-${i}`)} style={{ flexShrink: 0, padding: '3px 8px', background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: 6, fontSize: 11, color: '#64748b', cursor: 'pointer', fontFamily: 'inherit' }}>
                    ...
                    {!answers[q] && (
                      <div style={{ fontSize: 12, color: '#64748b', fontStyle: 'italic' }}>No answer yet — click Write All Answers above</div>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div style={{ background: '#0f1e35', border: '1px solid #1e3a5f', borderRadius: 12, overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: '1px solid #1e3a5f', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <a href="https://search.google.com/test/rich-results" target="_blank" rel="noreferrer" style={{ padding: '6px 14px', background: '#0a1628', color: '#94a3b8', border: '1px solid #1e3a5f', borderRadius: 8, fontSize: 12, fontWeight: 700, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                    🔍 Validate
                  </a>
                </div>
              </div>
              <pre style={{ margin: 0, padding: '16px 20px', fontSize: 12, color: '#94a3b8', overflowX: 'auto', maxHeight: 280, background: '#0a1628', lineHeight: 1.6 }}>
                {`<script type="application/ld+json">\n${schema}\n</script>`}
              </pre>
              <div style={{ padding: '12px 20px', background: 'rgba(59,130,246,0.05)', borderTop: '1px solid #1e3a5f', fontSize: 12, color: '#64748b' }}>
                💡 Paste this into the <code style={{ background: '#0a1628', padding: '1px 5px', borderRadius: 4 }}>&lt;head&gt;</code> tag of every service page — not just the homepage.
              </div>
            </div>
          )}
          ...
          {/* Platform Deployment */}
          <div style={{ background: '#0f1e35', border: '1px solid #1e3a5f', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #1e3a5f', fontWeight: 700, fontSize: 15 }}>
              🌐 Deploy to Voice Assistants
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 0 }}>
              {PLATFORMS.map((p, i) => (
                <div key={p.id} style={{ padding: '16px 20px', borderRight: i < PLATFORMS.length - 1 ? '1px solid #1e3a5f' : 'none', borderBottom: '1px solid #1e3a5f' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <span style={{ fontSize: 20 }}>{p.icon}</span>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 13 }}>{p.label}</div>
                      <div style={{ fontSize: 11, color: '#64748b' }}>{p.coverage} of voice searches</div>
                    </div>
                  </div>
                  <div style={{ fontSize: 12, color: '#64748b', marginBottom: 10, lineHeight: 1.5 }}>{p.action}</div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {p.id === 'google' && schema && (
                      <button onClick={() => copyText(schema, 'schema-google')} style={{ padding: '4px 10px', background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', color: '#94a3b8', fontFamily: 'inherit' }}>
                        {copied === 'schema-google' ? '✓ Copied' : '📋 Copy Schema'}
                      </button>
                    )}
                    {p.id === 'siri' && (
                      <>
                        <a href="https://mapsconnect.apple.com" target="_blank" rel="noreferrer" style={{ padding: '4px 10px', background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', color: '#94a3b8', textDecoration: 'none' }}>Apple Maps →</a>
                        <a href="https://biz.yelp.com" target="_blank" rel="noreferrer" style={{ padding: '4px 10px', background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', color: '#94a3b8', textDecoration: 'none' }}>Yelp →</a>
                      </>
                    )}
                    {p.id === 'alexa' && (
                      <a href="https://www.bingplaces.com" target="_blank" rel="noreferrer" style={{ padding: '4px 10px', background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', color: '#94a3b8', textDecoration: 'none' }}>Bing Places →</a>
                    )}
                    {p.id === 'bing' && (
                      <a href="https://www.bing.com/webmasters" target="_blank" rel="noreferrer" style={{ padding: '4px 10px', background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', color: '#94a3b8', textDecoration: 'none' }}>Bing Webmaster →</a>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div style={{ background: '#0f1e35', border: '1px solid #1e3a5f', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #1e3a5f' }}>
              ...
              <div style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>Write a 40–60 word answer that wins Position Zero and gets read aloud by voice assistants</div>
            </div>
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 6 }}>TARGET QUESTION</label>
                  <input
                    value={snippetQ}
                ...
                <div>
                  <label style={{ fontSize: 12, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 6 }}>TARGET KEYWORD</label>
                  <input
                    value={snippetKw}
              ...
              <button
                ...
                style={{ alignSelf: 'flex-start', padding: '9px 20px', background: writingSnippet ? '#1a2d4a' : 'linear-gradient(135deg, #8b5cf6, #7c3aed)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: writingSnippet ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
              >
              ...
              {snippet && (
                <div style={{ background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: 10, padding: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    ...
                    <button onClick={() => copyText(snippet, 'snippet')} style={{ padding: '4px 10px', background: '#0f1e35', border: '1px solid #1e3a5f', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer', color: '#94a3b8', fontFamily: 'inherit' }}>
          ...
          <div style={{ background: '#0f1e35', border: '1px solid #1e3a5f', borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: item.done ? 'rgba(16,185,129,0.08)' : '#0a1628', borderRadius: 8, border: `1px solid ${item.done ? 'rgba(16,185,129,0.2)' : '#1e3a5f'}` }}>
                  <span style={{ fontSize: 16 }}>{item.done ? '✅' : '⬜'}</span>
                  <span style={{ fontSize: 13, color: item.done ? '#10b981' : '#94a3b8', fontWeight: item.done ? 600 : 400, textDecoration: item.done ? 'line-through' : 'none' }}>{item.text}</span>
                  <span style={{ marginLeft: 'auto', fontSize: 11, color: '#64748b', fontWeight: 700 }}>Step {i + 1}</span>
                </div>
              ))}
            </div>
          </div>
